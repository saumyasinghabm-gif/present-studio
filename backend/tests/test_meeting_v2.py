...
def test_admission_request_enters_room_before_waiting_lobby():
    ...
    try:
        ...
        with patch.object(meeting_v2.sm.sio, "enter_room", new_callable=AsyncMock) as enter_room, patch.object(
...
def test_admission_decision_is_broadcast_as_backup_for_waiting_guest():
    ids = _create_meeting(approved=False)
    try:
        meeting_v2.sm.controller_sids[ids["presentation"]] = {"host-sid"}
        ...
        assert any(call.kwargs.get("room") == "audience-sid" for call in decision_calls)
        assert any(call.kwargs.get("room") == ids["presentation"] for call in decision_calls)
    finally:
        _cleanup(ids)
