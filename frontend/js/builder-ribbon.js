(() => {
  ...
  const insert = [
    ...
    group("WordArt Format", `<div class="tool-stack wordart-tools"><div class="tool-row"><select class="ribbon-select wordart-style-select" id="wordArtStyle" aria-label="WordArt style"><option value="custom">Custom</option><option value="gold">Gold</option><option value="ocean">Ocean</option><option value="outline">Outline</option><option value="neon">Neon</option></select><label class="wordart-color" title="WordArt fill"><span>Fill</span><input class="builder-color-input" id="wordArtFill" type="color" value="#f5c842" aria-label="WordArt fill color"></label><label class="wordart-color" title="WordArt outline"><span>Line</span><input class="builder-color-input" id="wordArtOutline" type="color" value="#8a5b00" aria-label="WordArt outline color"></label></div><div class="tool-row"><label class="wordart-width"><span>Outline</span><input id="wordArtOutlineWidth" type="range" min="0" max="12" step="1" value="2" aria-label="WordArt outline width"><output id="wordArtOutlineValue">2 px</output></label><label class="wordart-shadow"><input id="wordArtShadow" type="checkbox" checked><span>Shadow</span></label>${iconTool("word-art-reset", "bi-arrow-counterclockwise", "Reset WordArt formatting")}</div></div>`, "wordart-format-group"),
    group("Shapes", `<div class="shape-tool-grid">${tool("insert-shape", "bi-square", "Rectangle", "", 'data-shape="rectangle"')}${tool("insert-shape", "bi-circle", "Circle", "", 'data-shape="circle"')}${tool("insert-shape", "bi-triangle", "Triangle", "", 'data-shape="triangle"')}${tool("insert-shape", "bi-arrow-right", "Arrow", "", 'data-shape="arrow"')}${tool("insert-shape", "bi-slash-lg", "Line", "", 'data-shape="line"')}</div>`),
    group("Media", `<label class="tool-button tool-tall file-ribbon-tool"><input type="file" accept="image/*" data-media-input="image"><i class="bi bi-image"></i><span>Image</span></label><label class="tool-button tool-tall file-ribbon-tool"><input type="file" accept="video/*" data-media-input="video"><i class="bi bi-play-btn"></i><span>Video</span></label><label class="tool-button tool-tall file-ribbon-tool"><input type="file" accept="audio/*,.mp3" data-media-input="audio"><i class="bi bi-volume-up"></i><span>Audio</span></label><div class="tool-stack media-options-stack"><button class="tool-button fit-media-tool" type="button" data-builder-action="fit-media"><i class="bi bi-arrows-fullscreen"></i><span>Fit without crop</span></button><small class="media-limit-note">Maximum upload: 100 MB</small></div>`)
  ].join("");
  ...
  ribbon.innerHTML = panel("home", home, true) + panel("insert", insert) + panel("transitions", transitions) + panel("animations", animations);
  ribbon.querySelector("#fontFamily").closest(".ribbon-group").id = "editorPanel";
})();
