(function () {
  ...
  window.PresentStudioApi = {
    ...
    resolveShareLink: (token) => request(`/api/presentations/shared/${encodeURIComponent(token)}`),
    ...
    getLiveMediaToken: (id, payload = {}) => request(`/api/presentations/${encodeURIComponent(id)}/live/media-token`, { method: "POST", body: JSON.stringify(payload) }),
    getScreenAccessRequirements: (id, token = "") => request(`/api/presentations/${encodeURIComponent(id)}/screen-access${token ? `?token=${encodeURIComponent(token)}` : ""}`),
    verifyScreenAccessCode: (id, token, screenAccessCode) => request(`/api/presentations/${encodeURIComponent(id)}/screen-access`, { method: "POST", body: JSON.stringify({ token, screenAccessCode }) }),
